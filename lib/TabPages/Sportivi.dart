...
class SportiviPage extends StatefulWidget {

  final FirebaseAuth auth;
  const SportiviPage({Key key, this.auth}) : super(key: key);

  @override
  _SportiviPageState createState() => _SportiviPageState();
}

class _SportiviPageState extends State<SportiviPage> {
    ...
    return StreamBuilder(
      ...
      builder: (BuildContext context, snapshot) {
        ...
        if (user["attiva_filtri"]) {
          if ((segmentedControlGroupValue == 0 || segmentedControlGroupValue == 1) && tags_utente.length > 10) {
            var toRemove = [];
  ...
  Stream getData2() {
    ...
    switch (segmentedControlGroupValue) {
      ...
      case 2:
        ...
        break;
      case 3:
        return FirebaseFirestore.instance.collection('users').doc(widget.auth.currentUser.uid).collection('sportivi').snapshots();
        break;
    }
  }
